  ...
  return new Promise((resolve, reject) => {
    fs.mkdir(workspaceDir, { recursive: true }, (err) => {
      ...
      fs.writeFile(mainPath, code, (err) => {
        ...
        // Write input.txt with Unix-style line endings
        const sanitizedInput = stdin.replace(/\r\n/g, '\n');
        const inputPath = path.join(workspaceDir, 'input.txt');
        fs.writeFile(inputPath, sanitizedInput.endsWith('\n') ? sanitizedInput : sanitizedInput + '\n', (err) => {
          if (err) return reject(err);
          resolve();
        });
      });
...
function executeCode(workspaceDir) {
  return new Promise((resolve) => {
    // Wrap the command in double quotes for sh -c
    const cmd = [
      'docker', 'run', '--rm',
      '-v', `${workspaceDir}:/workspace`,
      '--workdir', '/workspace',
    ...
    const { execFile } = require('child_process');
    execFile(cmd[0], cmd.slice(1), { timeout: 3000, shell: true }, (error, stdout, stderr) => {
