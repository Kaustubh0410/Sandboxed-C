...
app.post('/api/run', async (req, res) => {
  ...
  try {
    ...
    // Execute code, optionally using input.txt if stdin was provided
    const executeResult = await executeCode(workspaceDir, !!stdin);
...
function executeCode(workspaceDir) {
  return new Promise((resolve) => {
    ...
    execFile(cmd[0], cmd.slice(1), { encoding: 'utf8', maxBuffer: 1024 * 1024, timeout: 3000 }, (error, stdout, stderr) => {
      const timedOut = error?.killed || false;
      const exitCode = error?.code || 0;

      resolve({
        stdout: stdout.toString().replace(/\r/g, ''), // remove CR if Windows
        stderr: stderr.toString().replace(/\r/g, ''),
        exitCode,
        timedOut
      });
    });
